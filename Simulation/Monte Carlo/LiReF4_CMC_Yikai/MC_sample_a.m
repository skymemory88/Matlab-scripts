function [Mx, My, Mz, coef, eSpin, nSpin, E_si, E_int] = MC_sample_a(const, ion, params, E_si, E_int, hamI, basis, coef, eSpin, nSpin)
persistent time
    ...
    for tt = 1:length(params.temp)
        ...
        parfor ff = 1:size(params.field,2)
            ...
            for tInt = 1:params.mIntv
                [~, ~, E_si(:,tt,ff), E_int(:,tt,ff), coef(:,:,tt,ff), eSpinT] = thermalize_a(ion, const, params,...
                    beta, E_si(:,tt,ff), E_int(:,tt,ff), hamI(:,:,tt,ff), coef(:,:,tt,ff), basis(:,:,tt,ff), eSpinT, nSpinT);
                % thermalize the nuclear spins
                if params.hyp
                    nSpinT = therm_nuc_a(const, beta, ion, params, ff, eSpinT, nSpinT);
                end
                % checkpoint
            ...
            % for trial = 1:5
            %     [~, ~, ~, Esi(:,tt,ff), coef(:,:,tt,ff), eSpinT] = thermalize_cluster_a(ion, const, params,...
