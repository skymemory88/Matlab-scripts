function [E_si, eSpin0, nSpin0, params, coef] = initialize_a(const, ion, params, basis, hamI)
switch params.init
    case 'disordered'
...
[params.nList, params.nVecs, params.nDists] = neighborList_a(params.pos, params.dpRng); % Build neighbor lists once

if params.useGPU  % Add a flag to control GPU usage
    gpuState = GPUSpinState(params, length(params.temp), size(params.field,2), true);
    gpuState.initialize(params, const.gfac);
    fprintf('GPU acceleration enabled for dipSum calculations\n');
    
    % Initialize GPU spin configurations
    for tt = 1:length(params.temp)
        for ff = 1:size(params.field,2)
            gpuState.updateSpins(eSpin0(:,:,tt,ff), tt, ff);
        end
    end
    params.gpuState = gpuState;
    fprintf('Initialization complete with GPU support!\n')
else
    params.gpuState = [];
    fprintf('GPU acceleration disabled - using CPU calculations\n');
end
