...
% iState: initial spin configuration: "disordered", "ordered"
...
% saveOpt: saving option of data (True/False)
    ...
    if loadOpt % continue simulation from previous file
        if isfile(params.DataFile)
            ...
            if params.useGPU == true
                % Initialize GPU state after loading
                gpuState = GPUSpinState(params, length(params.temp), size(params.field,2), true);  % true = double precision
                gpuState.initialize(params, const.gfac);
                params.gpuState = gpuState;

                % Initialize GPU spin configurations
                for tt = 1:length(params.temp)
                    for ff = 1:size(params.field,2)
                        if exist('meas', 'var')
                            params.gpuState.updateSpins(meas.eSpin(:,:,tt,ff), tt, ff);
                        else
                            params.gpuState.updateSpins(init.eSpinT(:,:,tt,ff), tt, ff);
                        end
                    end
                end
            end
        ...
        %% setup the current measurement
        switch mion % Ions' proportions
            case 'Er'
                ion.prop(1) = 1; % LiErF4
        ...
        switch mion
            case 'Er'
                Bfield = 0;
                params.temp(1:10) = linspace(0.28, 0.55, 10);
                % params.temp(1:3) = linspace(0.02,0.06,3);
                % params.temp(4:18) = linspace(0.07,0.16,15);
                % params.temp(19:22) = linspace(0.18,0.25,4);
            ...
            case 'Ho'
                ...
                params.temp = linspace(0.05, 1.7, 24);
                Bfield = 0;
                theta = pi/2; % out-of-plane angle (radian) from ab plane
                phi = 0; % in-plane angle (radian) from a/b axis
            otherwise
        ...
        params.tEQ = 1e3; % Thermalization steps (in unit of full lattice sweep)
        params.sampSize = 100; % Sampling size
        params.mIntv = 1e3; % Interval between measurements
        params.sIntv = 20; % data saving interval
        params.pt_intv = 90; % Interval between parallel temperature trials
        params.tEQ = params.tEQ - mod(params.tEQ, params.pt_intv); % Round the steps to integers
        params.trap_lim = 20; % thermalization local minima trap limit
        % params.mIntv = params.mIntv - mod(params.mIntv, params.pt_intv);

        % construct the lattice
        params.mion = mion; % Rare earth ion specis
        ...
        params.dpRng = 15;  % dipole summation range (in unit of unit cells)
        params.dpRng = params.dpRng * max(diag(ion.abc{ion.idx}));  % [Angstrom] dipole summation range
        params.useGPU = true; % use GPU for diple summation
        params.pos = lattice(ion.abc{ion.idx}, ion.tau, params); % lattice site coordinates
        break;
    end
end

% Pre-compute Luijten–Blöte cluster look-up table
params = cluster_init_a(params, const);

% project the electronic hamiltonian to Ising space
[~, hamI, basis, ~, ~, ~] = Ising_proj(const, ion, params); % Projection to Ising space (2 x 2)

% Initialize the configuration for new simulations
if  ~loadOpt
    [E_si, eSpin0, nSpin0, params, coef] = initialize_a(const, ion, params, basis, hamI);
end
...
while SampSz <= params.sampSize
    if mod(SampSz, 5) == 0
        sprintf('Current Sample size: %d, progress: %.2f\n', SampSz, SampSz/params.sampSize*100);
    end
