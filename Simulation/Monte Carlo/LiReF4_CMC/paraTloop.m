...
interval = 50*params.N_Er;
tempMark = zeros(floor(params.Nitermeas/interval),1);
...
spmd
    ...
    % for ii=1:size(temp,2) % single thread for-loop for debugging
        t=tic;  
        disp(['T = ',num2str(temp(ii)), ', Field = [', num2str(params.field(1)), ', ', num2str(params.field(2)), ', ', num2str(params.field(3)), '] ', num2str(labindex,'Worker ID: %u')]);
    ...
    %     [relaxE(ii,:),Egs,Egs2,mag,~,lat_mom{ii,1}]=LiIonsF4_MC(ion,params,inter,lattice,temp(ii),E_0,EQlat_mom{ii},numWkrs,worker.ID,field_change);
        [relaxE,Egs,Egs2,mag,~,lat_mom,tempMark]=LiIonsF4_MC(ion,params,inter,lattice,temp(ii),E_0,EQlat_mom{ii},field_change,numWkrs,interval,tempMark);    
...
%         malt(ii)=mean(sqrt(altx.^2+alty.^2));
        malt=mean(sqrt(altx.^2+alty.^2));

        if(temp(ii)~=0)
%             C_v(ii)=(bestE2(ii)-(bestE(ii)^2))/((1/11.6)*(temp(ii)^2)); % kB~1/11.6 [meV/K]
            C_v = (bestE2-(bestE^2))/((1/11.6)*(temp(ii)^2)); % kB~1/11.6 [meV/K]
        end
