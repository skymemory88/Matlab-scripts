...
function [Egs,Egs2,bestE,bestE2,acc_rate,C_v,malt,lat_mom,params,tempf]=paraTloop(ion,params,inter,lattice,EQlat_mom)
...
index = 1;
acc_rate = double.empty(block,int8(params.Nitermeas/params.meas_intv),0);
tempMark = ones(block,int8(params.Nitermeas/params.pt_intv)+1);
...
%     for ii=1:size(temp,2) % single thread for-loop for debugging
    ...
    %     [relaxE(ii+(labindex-1)*block,:),Egs,Egs2,mag,~,lat_mom{ii+(labindex-1)*block,1}]=LiIonsF4_MC(ion,params,inter,lattice,temp(ii+(labindex-1)*block),E_0,EQlat_mom{ii+(labindex-1)*block},numWkrs,worker.ID,field_change);
        [Egs,Egs2,acc_rate(index,:,1),mag,~,lat_mom,tempMark(index,:),tempf(index,1),~]=LiIonsF4_MC(ion,params,inter,lattice,temp(ii+(labindex-1)*block),E_0,EQlat_mom{ii+(labindex-1)*block},field_change,numWkrs,tempMark(index,:),prob(index,:));    
...
%             C_v(ii+(labindex-1)*block)=(bestE2(ii+(labindex-1)*block)-(bestE(ii+(labindex-1)*block)^2))/((1/11.6)*(temp(ii+(labindex-1)*block)^2)); % kB~1/11.6 [meV/K]
            C_v(index,1) = (bestE2(index,1)-(bestE(index,1)^2))/((1/11.6)*(temp(ii+(labindex-1)*block)^2)); % kB~1/11.6 [meV/K]
        end
...
clearvars -except Egs Egs2 acc_rate bestE bestE2 C_v malt lat_mom params tempf mark
end