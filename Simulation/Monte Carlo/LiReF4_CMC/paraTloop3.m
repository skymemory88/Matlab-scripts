...
function [Egs_final,Egs2_final,bestE,bestE2,acc_rate,Marker,C_v,C_fdt,malt,lat_mom,params]=paraTloop3(ion,params,inter,lattice,EQlat_mom)

temp = params.temp'; % temperature points at which measurements to be taken
N_meas = params.Nitermeas; % Number of the measurements per temperature
N_stats = params.Niterstat; % Number of statistical points used in one measurement
...
spmd
    for ii = labindex:numWkrs:size(temp,1) % for temperature assignment 1
%     for ii = 1:block % for temperature assignment 2
%     for ii = 1:size(temp,2) % single thread for-loop for debugging
%         t=tic;  
        temperature = temp(ii); % temperature assignment 1: each core moves forward by one stride equivalent to the number of workers, and the herd move together by strides
%        temperature = temp(ii+(labindex-1)*block); temperature assignment 2: each core moves forward by one point but separated from each other by a stride of the number of cores
        ...
        lat_mom = EQlat_mom{ii}; % temperature assignment 1
%         lat_mom = EQlat_mom{ii+(labindex-1)*block}; % temperature assignment 2
...
%             [E_0,Egs(jj,index,1),Egs2(jj,index,1),~,mag,lattice,lat_mom,~,temp(ii+(labindex-1)*block),~] = rand_walk(N_stats,inf,ion,params,inter,lattice,temp(ii+(labindex-1)*block),E_0,lat_mom,field_change,numWkrs,tempMark(index,:),prob(index,:));     
    ...
    %     angl(:,ii) = arrayfun(@(lat) atan2(lat.mom(2),lat.mom(1)),templattice); % Vectorized from original
        ...
        index = index + 1;
%         toc(t);
        fprintf('Total parallel tempering attempts: %d.\n', p_count)
    end
end
...
for ii = 1:N_meas
    Egs_final(ii,:) = reshape(Egs_temp(:,:,ii),size(params.temp)); % for temperature assignment 1
    Egs2_final(ii,:) = reshape(Egs2_temp(:,:,ii),size(params.temp)); % for temperature assignment 1
%     Egs_final(ii,:) = reshape(Egs_temp(:,:,ii)',size(params.temp)); % for temperature assignment 2
%     Egs2_final(ii,:) = reshape(Egs2_temp(:,:,ii)',size(params.temp)); % for temperature assignment 2
end
...
acc_rate = acc_rate_temp; % for temperature assignment 1
% acc_rate = acc_rate_temp'; % for temperature assignment 2

malt = squeeze(malt'); % for temperature assignment 1
% malt = squeeze(malt); % for temperature assignment 2
malt = malt(:);
...
clearvars -except Egs_final Egs2_final acc_rate Marker bestE bestE2 C_fdt C_v malt lat_mom params
end