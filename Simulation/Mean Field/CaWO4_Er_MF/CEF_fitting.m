...
elseif lsqAvailable
    [freeOpt, resnorm, ~, exitflag, output] = lsqnonlin(objective, free0, freeLower, freeUpper, lsqOpts);
    output.phase = 'lsqnonlin';
else
...
fprintf('Max |Δg/g|: %.2e (tolerance %.1e)\n', maxGError, gTol);
...
function res = fitResidual(freeB, baseB, mask, targetG, L, S, Ix, Iy, Iz, Eobs16c, wG, wE, gTol, fieldData)
    ...
    gres = gres / gTol;  % express g residual in units of the tolerance
...
function fieldRes = computeFieldResidual(fieldData, Jproj, Hhf_unit, A0_opt, Ix, Iy, Iz, L, S)
    ...
    for dd = 1:numel(fieldData)
        ...
        Q = getStructField(fd, 'Q', [0.0691E-4, 0.0691E-4, -0.1381E-4]);
