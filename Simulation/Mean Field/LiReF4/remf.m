...
for iterations = 1:NiterMax
    ...
    % Demagnetization effect on the external field [only for longitudinal field, see PRB 70, 144411 (2004)]
    Hd = mu0*muB/Vc * 10^30 * ion.prop'*ion.gLande .* mean(momente_old,1); % [T] demagnetization field per site
    Bz = Bfield(3) - ion.demag * diag(squeeze(demagn(3,3,1,1)))' .* Hd(3); % substract the demagnetization field
    if Bz >=0; hvec(3) = Bz; end % only apply when Bfield > demag_field
        ...
        for ii = 1:size(ion.name,1)
            if ion.prop(ii) > 0 % if the ion exists
                if ion.hyp(ii) == 0 % if there is no hyperfine interaction
                    [jx,jy,jz,energies(ionn,:),v] = MF_moments(hvec, h_mf(ii,:), t, ion.J(ii), ion.gLande(ii),...
                        ion.cf{:,:,ii}, ion.h4(ii));
                    ...
                    [jx,jy,jz,energies(ionn,:),v] = MF_moments_hyper(hvec, h_mf(ii,:), t, ion.J(ii), ion.gLande(ii),...
                        ion.nLande(ii), ion.cf{:,:,ii}, ion.A(ii), ion.I(ii), ion.h4(ii));
                ...
                if ion.hyp(ii) ~= 1 % if there is only partial hyperfine interaction
                    [jx,jy,jz,~,~] = MF_moments(hvec, h_mf(ii,:), t, ion.J(ii), ion.gLande(ii),...
                        ion.cf{:,:,ii}, ion.h4(ii));
                    ion.mom(ionn,:,ii) = update_moments([jx,jy,jz],evolution.(ion.name{ii}),ionn,iterations);
                end
...
% Ham = Hcf + Hzeeman; % Crystal field + Zeeman
% Ham = Hzeeman + Hint; % without crystal field term
Ham = Hcf + Hzeeman + Hint + H_h4; % full Hamiltonian
