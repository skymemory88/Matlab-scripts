...
chiq = zeros(3,3,length(freq_total(1,:)),size(qvec,1),length(fields(1,:)));
...
for jj=1:size(qvec,1)
    nn = 3; % number of moments in one unit cell
    D(:,:,:,:,jj)=dipole_direct(qvec(jj,:),dip_range,a) + exchange(qvec(jj,:),ion.ex(2),a,nn);
end
D=D*(6/5)^2*0.05368;
...
for ii = 1:length(fields(1,:))
    for kk = 1:length(freq_total(1,:))
        for nq = 1:size(qvec,1)
            ...
            for n = 1:N
                for m = 1:N
                    MM((n-1)*3+(1:3),(m-1)*3+(1:3)) = (chi0r(:,:,kk,ii) + 1i*chi0i(:,:,kk,ii))*D(:,:,n,m,nq);
                end
            end
             
            chi = chi0r(:,:,kk,ii) + 1i*chi0i(:,:,kk,ii); 
            chiq(:,:,kk,nq,ii) = 1/4*[eye(3) eye(3) eye(3) eye(3)]*((eye(size(MM))-MM)\([chi;chi;chi;chi]));
            
            rechi1x(kk,ii,1) =  real(chiq(1,1,kk,nq,ii));
            imchix(kk,ii,1) =  imag(chiq(1,1,kk,nq,ii));
            
            rechi1y(kk,ii,1) =  real(chiq(2,2,kk,nq,ii));
            imchiy(kk,ii,1) =  imag(chiq(2,2,kk,nq,ii));
            
            rechi1z(kk,ii,1) =  real(chiq(3,3,kk,nq,ii));
            imchiz(kk,ii,1) =  imag(chiq(3,3,kk,nq,ii));
        end
    end
end
rechi.x = rechi1x;
