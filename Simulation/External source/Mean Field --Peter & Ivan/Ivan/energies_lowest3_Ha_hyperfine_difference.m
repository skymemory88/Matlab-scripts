function energies_lowest3_Ha_hyperfine_difference
clear all
...
% A_Ho_1985=0.0066;
gLande=1.25;
hplanck = 4.13566733e-15; % Planck constant
...
%     eng(h,:)=diag(d)-min(diag(d));
    ...
    for j = 1:15
        diff(h,j)= ( eng(h,j+1)-eng(h,j) ) / 1e3 / hplanck / 1e9;
        diff2(h,1)= ( eng(h,10)-eng(h,7) ) / 1e3 / hplanck / 1e9;
        diff3(h,j)= ( eng(h,j+2)-eng(h,j) ) / 1e3 / hplanck / 1e9;
    end
...
for i=1:size(B,2)
 for jj=1:15 % NMR
%     for jj=1:8 % EPR
    energies(i,jj)=eng(i,jj+1)-eng(i,jj); % NMR, delta_m_nuclear=1, delta_m_electron=0
...
hfig2 = figure(9);
clf
ttitle='energies_lowest3_Ha_hyperfine_difference';
% ttitle='energies_lowest3_Ha_hyperfine';
p1=plot(B(1,:),diff(:,:),'r'); hold on;
p2=plot(B(1,:),diff2(:,:),'k');
% % % % p3=plot(B(1,:),diff3(:,:),'k');
...
ylabel('Energy difference (GHz)','FontSize',13)
ax=gca;
set(ax,'FontSize',13);
box on
set(p1,'linewidth',1,'Color',[35 107 142]/255,'linestyle','-')
set(p1(8),'linewidth',1,'Color',[255 0 0]/255,'linestyle','-')
set(p2,'linewidth',1,'Color',[255 0 0]/255,'linestyle','-')
