...
switch opt
    case 1
        ...
        ll = 1;
        for phi = sPHI
            [fields{ll},altJ_H{ll},J_H{ll},nmax{ll},ion{ll}] = LiYbF4_mf_sim(phi,Hz);
            ll = ll + 1;
        end

        % Make plots of Jx,Jy,Jz and staggered magnetisation
        option1(sPHI,fields,altJ_H,J_H,nmax,ion)
...
%         sHx = [0   0.05 0.1 0.15 0.2 0.25 0.5 1];
%         sHy = 0.*sHx;
%         sHz = 0.*sHx;
...
function option1(sPHI,fields,altJ_H,J_H,nmax,ion)
...
for ll = 1:length(fields)
    p1(ll)=plot( h1,        Hz,altJ_H{ll}(1,:,nion));
    p2(ll)=plot( h2,        Hz,altJ_H{ll}(2,:,nion));
    p3(ll)=plot( h3,        Hz,altJ_H{ll}(3,:,nion));
    p4(ll)=plot( h4,        Hz,J_H{ll}(1,:,nion));
    p5(ll)=plot( h5,        Hz,J_H{ll}(2,:,nion));
    p6(ll)=plot( h6,        Hz,J_H{ll}(3,:,nion));
...
curdir = cd;
cd('C:\Users\Peter\Documents\MATLAB\Matlab library\Draw 3d arrow in figures')
...
%     pause(1)
    ...
    save2latex(hfig,['H_' num2str(fields{nfields}(1)*1000,' %d') '_' num2str(fields{nfields}(3)*1000,' %d') '_3D'])
    delete(t)
    title('')
...
%                        ];
    ...
    temp=5:1:20; %  range of temperatures
    fields=[4;0;0];
end
...
function [fields,altJ_H,J_H,nmax,ion] = LiHoF4_mf_sim(Hx,Hy,Hz)
...
%                        ];
    ...
    temp=0.1:0.1:4; %  range of temperatures
    fields=[4;0;0];
end
...
% fileMF = fopen('C:\Users\ikovacev\Documents\Projects\LiHoF4_EPR_MF_calculations_and_literature\Ivan\mf.txt', 'w');
%         fclose(fileMF);
%         
[ion,history,E]=LiIonsF4(ion,temp,fields,demagn,alpha);
% 
%         fileMF = fopen('C:\Users\ikovacev\Documents\Projects\LiHoF4_EPR_MF_calculations_and_literature\Ivan\mf.txt', 'r');
% %         energies = fscanf(fileMF,'%f', [17 inf]);
% %         energies = fscanf(fileMF,'%f', [136 inf]);
%         energies = fscanf(fileMF,'%f', [15 inf]); % NMR
% %         energies = fscanf(fileMF,'%f', [8 inf]); % EPR
%         fclose(fileMF);
% 
%         hfig2 = figure (1283);
%         clf
%         hold on
%         set(hfig2,'position',[500 50 600 600])
% %         h1=plot(fields(1,:), energies); % 
% %             h1=plot(fields(1,:), energies(1:8,:),'r');  % EPR
%         h1=plot(fields(1,:), energies(1:7,:),'r');  % NMR low (only)
%         h2=plot(fields(1,:), energies(9:15,:),'b');  % NMR high
% %         h1=plot(temp, energies);
%         plot([min(fields(1,:)) max(fields(1,:))],[3.925 3.925],'k')
%         xlabel('Hx [T]','FontSize',20)
% %         xlabel('T [K]','FontSize',20)
%         ylabel('delta Energy / h [GHz]','FontSize',20)
%         ylim([0 10]);
% %         l1=legend(h1,'low E','Location', 'EastOutside'); % NMR
% %         l1=legend(h2,'high E','Location', 'EastOutside'); % NMR
% %         l1=legend('low E','low E','low E','low E','low E','low E','low E','high E','high E','high E','high E','high E','high E','high E','Location', 'EastOutside'); % NMR
%         tit=num2str(temp,'Hx Hcf+Hzeeman+Hdipole+Hyperfine T = %2.2g K');
% %         tit=num2str(fields(1),'T Hcf+Hzeeman-Hdipole H = %2.2g T');
%         title(tit,'FontSize',15)
%         saveplots(hfig2,tit)
%         
%         % NMR
%         fileNMR = fopen('C:\Users\ikovacev\Documents\Projects\LiHoF4_EPR_MF_calculations_and_literature\Ivan\testNMR.txt', 'a+');
%          fprintf(fileNMR,'%3.3f \t', temp);
%                 for i=1:8
%             line(i)=spec1d(fields(1,:), energies(i,:), 0.00000001);
%             xnew = [0:0.001:Hmax];
%             nline(i)=interpolate(line(i),xnew);
%             [H,E,err]=extract(nline(i));
%      
%             if  ~isempty(find(E<3.925, 1, 'first'))
%             nmr_index(i) = find(E<3.925, 1, 'first');
%             nmr_field(i) =H(nmr_index(i));
%             fprintf(fileNMR,'%3.3f \t', nmr_field(i));
%             E=flipud(E);
%             H=flipud(H);
%             nmr_index(i) = find(E<3.925, 1, 'first');
%             nmr_field(i) =H(nmr_index(i));
...
%             end
%                 end
%         for i=9:15 % higher energy levels - high T
%             line(i)=spec1d(fields(1,:), energies(i,:), 0.00000001);
%             xnew = [0:0.001:Hmax];
%             nline(i)=interpolate(line(i),xnew);
%             [H,E,err]=extract(nline(i));
