...
//     const int* type;
    ...
    for(i=0;i<Nions;++i){
        ...
        memcpy(&pos[3*i],mxGetPr(dummy),3*sizeof(double));
        dummy=mxGetField(m_ion_i,0,"mom");
        memcpy(&mom[3*i],mxGetPr(dummy),3*sizeof(double));
        dummy=mxGetField(m_ion_i,0,"staggfield");
        memcpy(&staggfield[3*i],mxGetPr(dummy),3*sizeof(double));   
        dummy=mxGetField(m_ion_i,0,"num");
        ion_types[i]=(int)*mxGetPr(dummy)-1;
     ...
     ion_F=malloc(sizeof(double)*6);
      ...
      for(i=0;i<6;++i){  
        ...
        ion_glande[i]=*mxGetPr(dummy);
        dummy=mxGetField(m_ion_i,0,"F");
        ion_F[i]=*mxGetPr(dummy);
    }
    
    /*copy parameters for the modified ion*/
    new_ion=(int)mxGetPr(m_new_ion)[0]-1;
    field=mxGetPr(m_field);
    inter=mxGetPr(m_inter);
    new_mom=mxGetPr(m_new_mom);
    size=(int)mxGetPr(m_size)[0];
    old_E_Zee=(double)mxGetPr(m_old_E_Zee)[0];
    new_E_Zee=(double)mxGetPr(m_new_E_Zee)[0];
