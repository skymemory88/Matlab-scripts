...
ct=0.05368; % =mu_0*mu_B^2/4pi ([meV*Ang^-3])
...
[n,m,l,k,h]=ndgrid(1:4,1:4,-L:L,-L:L,-L:L);
hklmn_d=[h(:) k(:) l(:) m(:) n(:)];
...
%Er
% V_uc=params.abc(1,1)*params.abc(2,2)*params.abc(3,3); % Unit cell volume
...
% demagn=ellipsoid_demagn(1);
 ...
 V=params.abc(1,1)*L*params.abc(2,2)*L*params.abc(3,3)*L; % Box volume
...
for i=1:size(hklmn_d,1)
    r_ij=[h(i);k(i);l(i)]+params.pos(:,n(i))-params.pos(:,m(i));
