function [d]=dipole_direct(ion,q,N)
...
for mt=1:nt
  ...
  rr=sum(r.^2,2);
  out_of_sphere=find(rr>(N*5.162)^2 | rr<0.01);
  ...
  for n=1:3
  for m=1:3
    d(n,m,nt,mt)=exp_qr*(3*r(:,n).*r(:,m)./rr25-delta(n,m)./rr15);
...
% coord = {'x','y','z'};
% ind = {'ion 1','ion 2','ion 3','ion 4'};
% [xind,yind] = meshgrid([1 2 3 4],[1 2 3 4]);
% xind = reshape(xind,[16 1]);
% yind = reshape(yind,[16 1]);
% 
% for i = 1:3
%     for j = 1:3
%         d1d = reshape(d(i,j,:,:),[16,1]);
%         figure;
%         scatter(xind,yind,5000,d1d,'filled','Marker','s')
%         title(strcat({'RPA: '},coord(i),{', '},coord(j)),'Interpreter','latex')
%         xticks(1:4)
%         yticks(1:4)
%         xticklabels(ind)
%         yticklabels(ind)
