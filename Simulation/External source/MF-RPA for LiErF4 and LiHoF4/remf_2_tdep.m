function [momente_er,momente_er_hyp,momente_ho,evolution]=remf_2_tdep(h,t,momente_er,momente_ho,exHo,exEr,xEr,xHo,ErHyp,HoHyp,withdemagn,alpha,renorm_Er,renorm_Ho)
...
%% Time-dependence ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
hh = 100/10000;                 % Field of the resonator ~1 Oe
freq = 0.0001e9;                     % Resonance field (GHz)      
tt = linspace(0,1/freq,100);    % Time interval over which to evaluate the wavefunctions
dt = tt(2) - tt(1);             % Step in time
hbar = 6.58211928e-13;          % Planck's constant/2pi (meV s)
...
% % At resonance
% freq = e0(2)*241.8e9;         % meV => GHz
% tt = linspace(0,4/freq,50);    % Time interval over which to evaluate the wavefunctions
% dt = tt(2) - tt(1);             % Step in time

disp([num2str(1) ' ' num2str(e{1}(1:10)')])

jx(1)=real(v{1}(:,1)'*Jxh*v{1}(:,1));
jy(1)=real(v{1}(:,1)'*Jyh*v{1}(:,1));
jz(1)=real(v{1}(:,1)'*Jzh*v{1}(:,1));

for n=2:length(tt)
    
    
    Ht = Ham0 + (-gLande*0.05788)*Jyh*hh*sin(2*pi*freq*tt(n));
    ...
    while m > 1
        for jj = 1:size(v{1},1)
            vf{m-1}(:,jj) = v{1}(:,1).*0;
            for ii = 1:size(v{1},1)
                vf{m-1}(:,jj) = vf{m-1}(:,jj) + vv(jj,ii)*exp(-1i*e{m}(ii)/hbar*dt)*vf{m}(:,ii);
            end
        end
        m = m - 1;
    end
%     disp([num2str(n) ' ' num2str(e{n}(1:10)')])

    jx(n)=real(vf{1}(:,1)'*Jxh*vf{1}(:,1));
    jy(n)=real(vf{1}(:,1)'*Jyh*vf{1}(:,1));
    jz(n)=real(vf{1}(:,1)'*Jzh*vf{1}(:,1));
    
    if mod(n,round(length(tt)/5)) == 1
        % Update progress
        disp(num2str(n/length(tt)*100,'%3.3d'))
    end
...
subplot(2,2,1), 
plot(2*pi*freq*tt,jx), 
ylabel('jx')

subplot(2,2,2), 
plot(2*pi*freq*tt,jy), 
ylabel('jy')

subplot(2,2,3), 
plot(2*pi*freq*tt,jz), 
ylabel('jz')

subplot(2,2,4), 
plot(2*pi*freq*tt,hh*sin(2*pi*freq*tt))
hold on
scatter(2*pi*freq*tt,hh*sin(2*pi*freq*tt))
title([num2str(freq/1e9) ' GHz, ' num2str(hh) ' T'] )
