function svec=cut_spec1d(svec,varargin)
%
...
for n=1:length(svec)
   ...
   for i=1:length(varargin)
      ...
      if xleft > xright
         ...
         svec(n).x(xci)=[]; svec(n).y(xci)=[]; svec(n).e(xci)=[]; 
         if ~isempty(svec(n).yfit), svec(n).yfit(xci)=[]; end
      else
         xcl=find(svec(n).x<xleft);
         xcr=find(svec(n).x>xright);
         svec(n).x([xcl;xcr])=[]; svec(n).y([xcl;xcr])=[]; svec(n).e([xcl;xcr])=[]; 
         if ~isempty(svec(n).yfit), svec(n).yfit([xcl;xcr])=[]; end
      end
