function [p, std]=speclsqr_win(x,y,err,pin,dpin,func,barriers,fcp)
%CK modified 2008;
% Version 3.beta
...
for iter=1:niter,
  ...
  %%%%CK
  dbarrier=barrier(:,2)-barrier(:,2);
  xd1=(pprev-barrier(:,1))./dbarrier;
  xd2=-(pprev-barrier(:,2))./dbarrier;
  chi_barrier=1+(xd1<0).xd1.^2+(xd2<0).xd2.^2;
  %%%%CK
  r=wt.*(y-fbest);
  sprev=sbest;
  sgoal=(1-stol)*sprev;
