function r=combine_spec1d(varargin)
%
...
% 'neutron' : provided monitor is defined through spec1d, calculates
%               counts/monitor and uncertainty/monitor
...
%
% DFM 1.4.98, HMR, NBC, BHL 20.11.2000, (neutron) PB 09.05.2018
x=[];
y=[];
e=[];
m=[];
...
% %    warning('Unrecognized method, using ''counts''') PB
   method='counts';
end
...
for i=1:length(x);
   if isempty(xcombi)	
      ...
      if x(i)-xcombi(1)>toll,
         ...
         elseif strcmp(method,'neutron') % Normalised counts, errors will be overwritten later
            xres=[xres;     sum(xcombi.*mcombi) /max(sum(mcombi),eps)];
            yres=[yres;     sum(ycombi) /sum(mcombi)];
            eres=[eres;sqrt(sum(ycombi))/sum(mcombi)];
...
if ~isempty(xcombi)
   ...
   elseif strcmp(method,'counts') % Normalised counts
      ...
      yres=[yres;     sum(ycombi.*mcombi) /max(sum(mcombi),eps)];
      eres=[eres;sqrt(sum(ycombi.*mcombi))/max(sum(mcombi),eps)];
   elseif strcmp(method,'neutron') % Correctly calculate errors for neutron data
      xres=[xres;     sum(xcombi.*mcombi) /max(sum(mcombi),eps)];
      yres=[yres;     sum(ycombi) /sum(mcombi)];
      eres=[eres;sqrt(sum(ycombi))/sum(mcombi)];
   elseif strcmp(method,'weight') % Weighted by inverse error
