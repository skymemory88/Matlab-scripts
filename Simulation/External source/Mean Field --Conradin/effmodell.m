...
if 1==1
  ...
  for n=0:nI-1
    ...
    for m=0:nI-1
      ...
      hIx(n*nJ+(1:nJ),m*nJ+(1:nJ))=eye(size(Jx))*Ix(n+1,m+1);
      hIy(n*nJ+(1:nJ),m*nJ+(1:nJ))=eye(size(Jx))*Iy(n+1,m+1);
      hIz(n*nJ+(1:nJ),m*nJ+(1:nJ))=eye(size(Jx))*Iz(n+1,m+1);
    end
  end
  hIJ=0.039/11.6*hIJ;
  Hcfz=hHcfz+hIJ;
  ...
  ix=v(:,1:16)'*hIx*v(:,1:16);
  iy=v(:,1:16)'*hIy*v(:,1:16);
  iz=v(:,1:16)'*hIz*v(:,1:16);
