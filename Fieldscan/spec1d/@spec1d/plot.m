function [hout,hbout,hcout]=plot(varargin)
%
...
for i=1:length(varargin)
    ...
    if isa(varargin{i},'spec1d')
        ...
        hll=line(x,y,'color',colororder(i,:),...
            'LineStyle','None','Marker',deblank(markerorder(1+mod(i-1,7),:)),...
            'MarkerSize',5,'MarkerFaceColor',colororder(i,:));
        if mod(i,2)==0
            set(hll,'markerfacecolor','none');
            %set(hll,'markeredgecolor','none')
        end
        set(hll,'ButtonDownFcn','editline(gco);');
        
        if ~isempty(yfit);
            %         hlf=line(x,yfit,'color',colororder(1+mod(i-1,6),:),...
            hlf=line(x,yfit,'color',colororder(i,:),...
