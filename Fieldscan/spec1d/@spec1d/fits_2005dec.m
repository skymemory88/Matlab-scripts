...
if nargin<=4, fcp=[0.0001 30 0.0001]; end
if isempty(fcp), fcp=[0.0001 30 0.0001]; end
...
if ischar(pin) | nargin==2
   if ~isempty(strmatch(func,peaklist))
      stats=peakm(s1(il),1);      
      pin=[stats(1)*stats(3) stats(2) stats(3) stats(5)];   
   end   
end   
...
%----- Fit data
   [p, sig]=speclsqr(x,y,e,pin,notfixed,func,fcp);
