...
%         H0(i) = HH(dB==value);    %use value matching to find the corresponding magnetic field from the original dataset (slow method)
    ...
    ff0 = interp1(H0,f0,Hx); %Create an interpolation of the resonant frequency along the DC magnetic field (Hx) axis
%     Hx = H0;
%     ff0 = f0; %Alernatively use the raw data for fitting
    ...
    for m = Hx
        ...
        Q(ll) = fbck.pvals(2)/fbck.pvals(3); %Calculate the quality factor
        chi(ll) = 1/Q(ll);
        ll = ll + 1;
