function [y, name, pnames, pin]=strerfit(x, p, flag)

%mit extinction relief
if nargin==2
    global hklglobal iipara
    y=0*x;
    for n=1:size(hklglobal,1)
        [Fs,e,lorentz]=strfm(hklglobal(n,:),p(4:length(p)));
        Fsp=Fs-(e*Fs)*e';
        nucb=extinctioninv(p(3),iipara(n)*lorentz);
        %test(iipara(n)*lorentz-extinction(p(3),nucb)*nucb)/nucb;
        y(n)= abs(p(1))*real(Fsp'*Fsp)+nucb;
        y(n)=extinction(p(2),y(n))*y(n)/lorentz;
    end
else
...
function ex=extinction(alpha,x)
ex=1/sqrt(1+2*alpha*x+0.5*alpha^2*x.^2./(1+0.2*alpha*x));
return

function x=extinctioninv(alpha,y)
pol=[-1*0.2*alpha,-1+y^2*(2*alpha^2*0.2+0.5*alpha^2), y^2*2*alpha+0.2*alpha, y^2 ];
r=roots(pol);
x=max(r);
return