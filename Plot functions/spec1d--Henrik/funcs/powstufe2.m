function [y, name, pnames, pin]=powstufe2(x, p, flag)
...
% Author:  Conradin Kraemer
% Description:  Power law y=0 x>x0

if nargin==2
	y= -p(1)*(x<p(4)).*(x-p(2))+(x<=p(5)).*(-p(3)*(x>=p(4)).*(x-p(4))-p(1)*(x>=p(4)).*(p(4)-p(2)))+(x>p(5)).*(-p(3)*(x>=p(4)).*(p(5)-p(4))-p(1)*(x>=p(4)).*(p(4)-p(2)));
else
