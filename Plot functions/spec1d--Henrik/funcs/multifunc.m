...
% You can use any expression, including user variables (for 'fn=' fields)
...
% Author:  EF <manuf@ldv.univ-montp2.fr> 07.97
...
if ischar(flag)
	...
	if flag(1) == 'f'
		...
		nhedit = length(hedit)/2;
		if i<= nhedit, set(hedit(2*i-1),'String',fn); end
	end
	if flag(1) == 'y'
		j = findstr(flag,'=');
		fn = flag((j(1)+1):length(flag));
...
% adds func fields if needed
   for i=(nhedit+1):nfunc
     hedit(2*i-1)=uicontrol(hmf_multif,...  
		'Style','edit',...    
   		'BackgroundColor',[1 1 1],...
   		'ForegroundColor',[0 0 0],...
     ...
     hedit(2*i)  =uicontrol(hmf_multif,...     
		'Style','text',... 
   		'ForegroundColor',[1 1 1],...
		'String',[ 'f' num2str(i) '='],...
		'Position',[5 (4+i*26+28+28+28) 35 16]);
   end
...
if (flag) % make eval string, call fcns...
   ...
   for i=1:nhedit
	...
	if ~isempty(find(exprtype == [ 0 1 3 4 7 ]))
		...
		elseif (isg == 0)
			...
			fprintf(1,' Unknown variable (might produce error)\n');
			if ~isempty(fname)
	...
	elseif ~isempty(find(exprtype == [ 2 5 6 ]))
		...
		if ~fy
			...
			if isempty(fy)
				...
				if ~isempty(fpnames)
					set(hedit(2*i-1),'String',[ word1 ' ' fpnames ]);
				end
			else
				word1 = 'user';
   ...
   if (exist('mf_mftmp.m','file') == 2)
	if flag 
		[y, name, pnames, pin] = feval('mf_mftmp',x,p,flag);
	else
