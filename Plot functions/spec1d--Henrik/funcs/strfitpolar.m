function [y, name, pnames, pin]=strfitpolar(x, p, flag)
...
rpt=zeros(3,4);
for nmp=1:length(mp)
    rpt=rpt+mp(nmp)*polarglobal(:,:,nmp);
end
for nt=1:4
    spin(:,nt)=rpt(1,nt)*[cos(rpt(2,nt))*sin(rpt(3,nt));sin(rpt(2,nt))*sin(rpt(3,nt));cos(rpt(3,nt))];
end
