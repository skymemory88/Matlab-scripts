...
fid=fopen(filename);
fseek(fid,fpos-length(['#S ' num2str(scan) ' ']),'bof');
com=fgets(fid);

% Sometimes, ffind misses by a few lines - in this case we rewind a bit.
if isempty(strmatch(['#S ' num2str(scan) ' '],com))
  fseek(fid,fpos-100,'bof');
  while ~feof(fid)
    com=fgets(fid);   
    if strmatch(['#S ' num2str(scan) ' '],com)
       break
    end
  end
end

com(1:length(['#S ' num2str(scan) ' ']))=[] % This line contains the scan command issued
