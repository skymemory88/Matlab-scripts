...
% line below was put in may 99 to read data from v2-flex at HMI.
%fpos=ffind(filename,'PNT')-1;
%fseek(fid,fpos,'bof');
%r=fgets(fid)
% Three extra reads for the banipo_in22_2000oct data.
%r=fgets(fid);
%r=fgets(fid);
%r=fgets(fid);
%head=fgets(fid);
...
% ffind seems to work crap with newer versions of matlab. do it 'by hand'
r=fgets(fid);
while strncmp(r,'DATA_:',6)==0 & ~feof(fid)
  r=fgets(fid);
end
head=fgets(fid);
head=['  ' head '  '];
