function [xcolo, ycolo, ecolo, mcolo, xlab, ylab, scano, normo,keep,flipo ]=mf_coldg(collist, ncol,xcol,ycol,ecol,mcol,scan,normi,Fstate)
%
...
if exist('Fstate') & ~isempty(Fstate)
   Fnum = 50;
   if size(Fstate,2) > 1
	plist = sprintf('F1=%d,F2=%d',Fstate(1,1),Fstate(1,2));
   	for i=2:size(Fstate(:,1)), 
plist=[plist, sprintf('|F1=%d,F2=%d',Fstate(i,1),Fstate(i,2))]; end
   else
	plist = num2str(Fstate(1));
	if length(Fstate) > 1
	plist = [ plist sprintf('|%d',Fstate(2:end)) ];
	end
   end;
else
    Fnum = 0;
...
if ~isempty(hmf_load)
   ...
   set(hmf_load,'Position',[400 400 174 207 + Fnum]);   %--- Reset window size and button positions
...
%------- Create figure window -------------------------------------
   hmf_load=figure('Position',[400 400 174 207+Fnum],...
   		'Tag','mf_coldlg',...
   		'MenuBar','none',...
   		'Name','MFIT: Col Select',...
...
if exist('scan') scano = scan; else scano = []; end
if (~exist('xcol') | isempty(xcol))   xcol=get(hx,'Value'); end
if (~exist('ycol') | isempty(ycol))   ycol=get(hy,'Value'); end
if (~exist('ecol') | isempty(ecol))   ecol=get(he,'Value'); end
if (~exist('mcol') | isempty(mcol))   mcol=get(hm,'Value'); end
if (~exist('scan') | isempty(scan))   scan=get(hs,'String'); end
if (~exist('normi') | isempty(normi))  normi=get(hn,'String'); end
