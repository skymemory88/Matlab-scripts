...
%
%function [x, y, err, xlab, ylab]=tasbatch(filespec)
%
% MVIEW/MFIT load routine for TASCOM data files. 
% (old and new type files)
...
disp([ 'Loading TASCOM data : ' filename ]);
[data,datastr]=ntasdata(filename);
if isempty(data)
   [data,datastr]=otasdata(filename);
end
...
if gui
	...
	if (~strcmp(scantmp,scan)) % need to reach a new part of file 
		...
		[x, y, err, xlab, ylab]=tasbatch(filespec);
		return
	end
end
