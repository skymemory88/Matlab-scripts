...
% changed +ki to +kf in line below: HMR 2001 may
      kf=sscanf(header((kfpos(1)+kf):length(header)),'%f');
		if ~silent
		fprintf(1,'KF = %.2g A-1\n',t);
		end
	end
...
% NExt line to avoid problem if same variable is written twice.
xcol=xcol(1);
...
if (ecol == ncolumns+1)
	err=sqrt(y);%+(y==0);			% ...root(y) errors requested        
