function [x, y, err, xlab, ylab]=e4batch(filespec)
%
%function [x, y, err, xlab, ylab]=hmibatch(filespec)
%
% MVIEW/MFIT load routine for HMO data files. 
...
% MZ 11.10.95 and DFM 2.11.95, EF 4.09.97, HMR 16.8.98
%
...
gui = 0;		% no gui choice
quiet = 1; % Blabber about the loading progress
...
i=strmatch('X=',fspec);
if ~isempty(i)
	xname=deblank(fspec(i(1),3:nchars)); end
i=strmatch('Y=',fspec);
if ~isempty(i)
	yname=deblank(fspec(i(1),3:nchars)); end

i=strmatch('M=',fspec);
if ~isempty(i)
	mname=deblank(fspec(i(1),3:nchars)); end

i=strmatch('N=',fspec);
if ~isempty(i)
	normf=deblank(fspec(i(1),3:nchars)); end

i=strmatch('E=',fspec);
if ~isempty(i)
	ename=deblank(fspec(i(1),3:nchars)); end

i=strmatch('S=',fspec);
if ~isempty(i)
	scan=deblank(fspec(i(1),3:nchars)); end
...
if ~quiet
  disp([ 'Loading HMI data : ' filename ]);
end  
[data,datastr]=e4data1(filename);
...
if isempty(ecol)
	if strcmp(ename,'sqrt(y)') ecol = ncolumns+1;
	else ecol = ncolumns+2; end
end

%===== GUI option call  ============================

if gui
	[xcol, ycol, ecol, mcol, xlab, ylab, scantmp, normf,keep]=mf_coldg(datastr, ncolumns,xcol,ycol,ecol,mcol,scan,normf);
	...
	if (~strcmp(scantmp,scan)) % need to reach a new part of file 
		...
		[x, y, err, xlab, ylab]=hmibatch(filespec);
		return
	end
end
...
if (ecol == ncolumns+1)
	err=sqrt(y);			% ...root(y) errors requested        
