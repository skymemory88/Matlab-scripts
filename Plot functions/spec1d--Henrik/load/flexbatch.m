...
disp([ 'Loading FLEX data : ' filename ]);
[data,datastr,pres,pscan,stepvec,stepstr,com,header,dathead,flip]=flexdata(filename);
...
if (ki == 0)	
	...
	if ~isempty(kipos)
		ki = findstr(header(kipos(1):length(header)),'=');
		ki=sscanf(header((kipos(1)+ki):length(header)),'%f');
		if ~silent
		fprintf(1,'KI = %.2g A-1\n',t);
		end
	end
...
if (kf == 0)	
	...
	if ~isempty(kfpos)
		kf = findstr(header(kfpos(1):length(header)),'=');
		kf=sscanf(header((kfpos(1)+ki):length(header)),'%f');
		if ~silent
		fprintf(1,'KF = %.2g A-1\n',t);
		end
	end
...
if ~isempty(PALcol)
	...
	for i = 1:pal
		...
		if ~isempty(F1col), f1 = data(pstate(1),F1col); else f1 = 0; end
		if ~isempty(F2col), f2 = data(pstate(1),F2col); else f2 = 0; end
		Fstate = [Fstate; f1 f2 ];	% list of polarisation states used during scan
	end;
	end
else Fstate = [];
end
...
if gui
	...
	if (~strcmp(scantmp,scan)) % need to reach a new part of file 
		...
		[x, y, err, xlab, ylab,monitor,optpars]=flexbatch(filespec);
		return
	end
end
...
%----- Create labels
   ...
   ymon=[' (per ' num2str(mean(monitor)) ' Mon)*' num2str(normf)];
   ylab2=ymon;
