function [x, y, err, xlab, ylab,monitor]=parbatch(filespec)
%
%function [x, y, err, xlab, ylab,monitor]=parbatch(filespec)
%
% MVIEW/MFIT load routine for Mfit PAR text data files. 
...
disp([ 'Loading Mfit PAR text data : ' filename ]);
[data,datastr]=pardata(filename);
...
if gui
	...
	if (~strcmp(scantmp,scan)) % need to reach a new part of file 
		...
		[x, y, err, xlab, ylab]=parbatch(filespec);
		return
	end
end
