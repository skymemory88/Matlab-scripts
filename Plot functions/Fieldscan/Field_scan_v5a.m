...
switch Options.fitfunc % Pick fitting function from either (1) custom function of (2) spec1d
    case 1 %Option 1: Custom function by Input-output formalism
        ...
        H_l = 0;
        H_h = 2;
        fr = length(Hx(Hx >= H_l & Hx <= H_h));
        kpe = double.empty(fr,0);
        w0 = double.empty(fr,0);
        B0 = double.empty(fr,0);
        parfor ii = 1:fr
            zq(:,ii) = zq(:,ii)-max(zq(:,ii)); % Normalize the data to baseline
            param = [FWHM(ii) ff0(ii) 0 Brf 1]; % starting value for Param = {'kpe', 'w0', 'Gc', 'Br', 'gma'}
%             Set up boundaries for the fitting parameters
            bound_l = [0 -inf 0 Brf 1]; % lower bound of fitting parameters
            bound_h = [inf inf 0 Brf 1]; % upper bound of fitting parameters
            fit = iptopt_0(yq(:,ii),zq(:,ii),Hx(ii),param,bound_l,bound_h,weight(:,ii));
            if mod(ii,20) == 0
        ...
        kpe = mean(kpe);
        w0 = mean(w0);
        H_cut = 2; % Starting field (T) of the data for fitting
        idx = find(Hx >= H_cut,1,'first');
        parfor ii = idx:length(Hx)
            zq(:,ii) = zq(:,ii)-max(zq(:,ii)); % Normalize the data to baseline
            freq_r = range(yq(:,ii)); % Frequency range of the sweep
            dw = freq_r*5e-2;
            param = [kpe w0 gc Brf 1e3]; % starting value for Param = {'kpe', 'w0', 'Gc', 'Br', 'gma'}
%             Set up boundaries for the fitting parameters
            bound_l = [kpe*0   w0-dw 1e-5 Brf 1e-4]; % lower bound of fitting parameters
            bound_h = [kpe*5e2 w0+dw 1e-1 Brf 1e4]; % upper bound of fitting parameters
            fit = iptopt_0(yq(:,ii),zq(:,ii),Hx(ii),param,bound_l,bound_h,weight(:,ii));
...
% figure
plot(Hx(idx:round(length(Hx)/200):end),ff0(idx:round(length(Hx)/200):end),'or','MarkerSize',2,'MarkerFaceColor','red');
...
Qplot2 = plot(Hx(idx:round(length(Hx)/100):end), Qf(idx:round(length(Qf)/100):end),'o-','MarkerSize',2);
...
if Options.fitfunc == 1
    figure
%     plot(Hx,Gc,'-o')
    plot(Hx(idx:end),medfilt1(Gc(idx:end),order),'-o')
    xlabel('Field (T)')
    ...
    yyaxis right
%     plot(Hx,gamma,'-s')
    plot(Hx(idx:end), medfilt1(1./gamma(idx:end),order),'-s')
