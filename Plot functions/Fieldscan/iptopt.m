function [fitresult, gof] = iptopt(x, y, field, P0, low, upr)
%CREATEFIT(omega,S11)
...
ft = fittype( @(kpe, w0, x1, x2, Br, gma, field, x) mag2db(abs(1+kpe./(1i*(x-w0) - kpe + (x1+1i*x2)/(1i*(field-Br)+gma)))),...
    'independent', {'x'}, 'dependent', {'y'}, 'coefficients',{'kpe', 'w0', 'x1', 'x2', 'Br', 'gma'}, 'problem', {'field'});
opts = fitoptions( 'Method', 'NonlinearLeastSquares');
opts.Display = 'Off';
opts.StartPoint = [P0(1) P0(2) P0(3) P0(4) P0(5) P0(6)];
opts.Lower = [low(1) low(2) low(3) low(4) low(5) low(6)];
opts.Upper = [upr(1) upr(2) upr(3) upr(4) low(5) low(6)];
