function [fitresult, gof] = str_cpl_ufit(H0, f0, init, bdl, bdh, weight, plt)
%CREATEFIT(H0,F0)
...
ft = fittype( 'wc + slope*(x-x0)./2 + sqrt((slope*(x-x0)).^2 + 4*g^2)./2', 'independent', 'x', 'dependent', 'y');
...
if plt
    ...
    legend( h, 'f0 vs. H0', 'Upper branch fit', 'Location', 'NorthEast', 'Interpreter', 'none' );
    % Label axes
