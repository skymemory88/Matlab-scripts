...
ft = fittype( '((slope*(x-x0))^2 + gamma^2) / (2*gc^2*gamma + kappa*( (slope*(x-x0))^2 + gamma^2))', 'independent', 'x', 'dependent', 'y');
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [init(1) init(2)  init(3)  init(4)  init(5)]; % [slope x0 gamma gc kappa]
